...
(defn command-send-status-icon [confirmed? outgoing]
  (merge
...
(def command-request-amount-text
  {:font-size   22
   :font-weight "500"
   :color       colors/black})
...
(defn command-send-currency-text [outgoing]
  {:font-size   22
   :margin-left 4
   :opacity     (if outgoing 1 0.4)
   :color       (if outgoing colors/wild-blue-yonder colors/blue)})
